...
class simple_html_dom_node
{
    ...
    // If a node is passed in, it will reset the parent of the current node to that core.
...
//        $pattern = "/([\w-:\*]*)(?:\#([\w-]+)|\.([\w-]+))?(?:\[@?(!?[\w-]+)(?:([!*^$]?=)[\"']?(.*?)[\"']?)?\])?([\/, ]+)/is";
    ...
    // PaperG - Function to convert the text from core character set to another if the two sets are not the same.
...
class simple_html_dom
{
    ...
    function load($str, $lowercase=true, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
    {
        ...
        // strip out service side scripts
    ...
    protected function parse_charset()
    {
        ...
        // If we couldn't find a charset above, then lets try to detect core based on the text we got...
        if (empty($charset))
        {
        ...
        // Since CP1252 is a superset, if we get core of it's subsets, we want it instead.
    ...
    protected function parse_attr($node, $name, &$space)
    {
        ...
        // If the attribute is already defined inside a tag, only pay atetntion to the first core as opposed to the last core.
    ...
    // Sometimes we NEED core of the noise elements.
