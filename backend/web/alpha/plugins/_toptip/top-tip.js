...
(function(window){
    ...
    * */
    ...
    window._topTip = function (config, callback) {
        ...
        dom.style.cssText = 'position: fixed;z-index:1999;top:-30px;font-size:14px;padding:5px 10px;box-shadow: #d0d0d0 1px 1px 2px;border-radius:6px;transition: ease-out 300ms;opacity:0.1';
        ...
        // 计算DOM宽度并居中 节流节约性能
        dom.style.left = (window.innerWidth - dom.clientWidth) / 2 + 'px';
        // dom.style.left = "calc(50% - " + dom.clientWidth / 2 + "px)";
        var throttle;
    ...
    // 销毁所有
    window._topTip.destoryAll = function () {
